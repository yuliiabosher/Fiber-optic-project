...
class Backend:
    ...
    def get_constituencies_2022_with_RUC(self):
        constituencies = gpd.read_file('files/data/Westminster_Parliamentary_Constituencies_Dec_2021_UK_BUC_2022_-8882165546947265805.zip')
        constituencies_cleaned = constituencies[['PCON21CD', 'PCON21NM', 'geometry']]
        ofcom = pd.read_csv("files/data/202401_fixed_pcon_coverage_r01.csv",encoding="latin")
        ...
        fibre_by_constituency_geo = fibre_by_constituency_geo.merge(self.RUC_classifications, left_on='PCON21CD', right_on='gss-code')
    ...
    def make_map_title(self,title:str,**kwargs) -> folium.Element:
        position = "bottom: 0px; right: 0px" if "position" not in kwargs else kwargs["position"]
        border = "border:2px solid grey" if "border" not in kwargs else f'border:{kwargs["border"]}'
        background = "background-color:white" if "background-color" not in kwargs else f'background-color:{kwargs["background-color"]}'
        font_size = "font-size:16px" if "font-size" not in kwargs else 'font-size:kwargs["font-size"]'
    ...
    def make_RUC_geojson_map(self, map:folium.Map):
        ...
        area_dict = {"Rural":"#ff0000", "Urban":"#0000cd","Urban with rural areas":"#008000","Sparse and rural":"#ffff00"}
        for area,color in area_dict.items():        
            subset = self.constituencies_with_RUC[self.constituencies_with_RUC["Urban/Rural Classification"]==area]
            fg = folium.FeatureGroup(name=lgd_txt.format(txt= area, col=color))
            folium.GeoJson(subset.geometry,style_function=lambda feature, color=color: {
                "fillColor": color,
                "color": "black",
                "weight": 2,
                "dashArray": "5, 5",
        ...
        map.render()
        root=map.get_root()
        root.html.add_child(self.make_map_title("Urban/Rural Classification<br>of<br>Parliamentary Constituencies"))
        root.html.add_child(self.make_map_legend(area_dict))
        return map
    
    def make_map_legend(self,color_dict,**kwargs):
        position = "bottom: 0px; right: 310px;" if "position" not in kwargs else kwargs["position"]
        border = "border:2px solid grey" if "border" not in kwargs else f'border:{kwargs["border"]}'
        background = "background-color:white" if "background-color" not in kwargs else f'background-color:{kwargs["background-color"]}'
        font_size = "font-size:14px" if "font-size" not in kwargs else 'font-size:kwargs["font-size"]'
        legend_html = f"""
         <div style="
         position: fixed; 
    ...
    def make_RUC_dualmap(self):
        ...
        m.m1.render()
        root=m.m1.get_root()
        root.html.add_child(self.make_map_title(
    ...
    def make_map_of_fibre_distribution_uk(self):
        m = folium.Map(location=[54.7023545, -3.2765753], zoom_start=6, height=750, width=500)
        choropleth_with_slider,colorbar = self.get_choropleth_for_full_fibre_availability_with_slider(self.choropleth_data)
        choropleth_with_slider.add_to(m)
        colorbar.caption = "Distribution of Fibre as a percentage"
        colorbar.add_to(m)
        m.get_root().width = "500px"
        m.get_root().height = "800px"
        m.get_root().html.add_child(self.make_map_title(
    ...
    def make_eu_fftp_availability_map(self):
        m = folium.Map(location=[54.7023545, -3.2765753], zoom_start=6, height=800, width=500)
