...
parser.add_argument(
    "--tags", required=False, help="Tags to be searched, default is `man_made=street_cabinet` , multiple tags should be separated with a comma.",
    default = "man_made=street_cabinet"
)

parser.add_argument(
    "--file_format", required=False, help="Format for files to saved. Default is `${country}_cabinets.json (must have ${country} in name",
    default =  "${country}_cabinets.json"
)

parser.add_argument("--map_name", required=False, help="Name of map file that shows search coverage. Default `cabinets.html`", default="cabinets.html")
...
def download_geojson(country:str,min_lon:float,min_lat:float,max_lon:float,max_lat:float,tags:str, api_keys:List) -> Tuple[Dict,Optional[str]]:
	'''rotate api_keys and get data'''
	try:
		api_keys.rotate()
		api_key = api_keys[0]
		logging.info("Using Api Key %s"%api_key)
		params = {"tags": tags,"api_key": api_key,"bbox": f"{min_lon},{min_lat},{max_lon},{max_lat}"}
...
def split_box(min_lon:float,min_lat:float,max_lon:float,max_lat:float) -> Tuple[Tuple[float],Tuple[float]]:
...
def get_coords(country:str) -> Tuple[float]:
	try:
		...
		min_lat,max_lat,min_lon,max_lon = [float(coord) for coord in query.raw["boundingbox"]]	    
	return min_lon,max_lon,min_lat, max_lat

def draw_box(min_lon:float,min_lat:float,max_lon:float, max_lat:float, m=folium.Map) -> bool:
	folium.GeoJson(shapely.box(min_lon,min_lat,max_lon,max_lat)).add_to(m)
	return True

def draw_errors(country,min_lon:float,min_lat:float,max_lon:float, max_lat:float, m=folium.Map) -> bool:
...
def main(api_keys:List,tags:str,file_format:str,map_name:str) -> None:
	...
	for country in eu_countries:
		...
		logging.info("Using bbox %s, %s, %s, %s for %s"%(min_lon,min_lat,max_lon, max_lat,country))
		if country in large_countries:
			...
			tbox_json_data,tbox_errors = download_geojson(country,tbox_min_lon,tbox_min_lat,tbox_max_lon,tbox_max_lat,tags, api_keys)
			...
			if tbox_json_data and not tbox_errors:
				...
				bbox_json_data,errors = download_geojson(country,bbox_min_lon,bbox_min_lat,bbox_max_lon,bbox_max_lat,tags, api_keys)
				...
				logging.info("Coordinates of Top Box for %s (%s,%s,%s,%s)"%(country,tbox_min_lon,tbox_max_lon,tbox_min_lat, tbox_max_lat))
				
		else:
			logging.info("Searching for %s for %s"%(tags,country))
			#if it is not a large country just download the whole bbox and write it out
			json_data,errors = download_geojson(country,min_lon,min_lat,max_lon,max_lat,tags, api_keys)
			write_to_file(file.substitute(country=country), json_data)
		
		if not errors:
			logging.info("Wrote search results for %s to file %s"%(country, file.substitute(country=country)))
			...
			if country in large_countries:
				...
				folium.PolyLine(locations=[median_coords], color='green', weight=3, opacity=1).add_to(m)
